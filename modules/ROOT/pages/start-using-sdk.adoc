...
For Ubuntu 18.10, and other Debian based releases unsupported by `couchbase-release-1.0-6-amd64.deb`, either replace the above _first-time setup_ instructions with the following -- using the _bionic_ repo of Ubuntu 18.04 to install on Ubuntu 18.10 Cosmic, Ubuntu 19.04 Disco, or other recent versions:
...
----
to get `PECL` installed.

NOTE: CentOS 7 ships with PHP 5.4, which must be upgraded to at least PHP 5.6 in order to run the Couchbase PHP SDK.
Doing this is beyond the scope of this quick start guide, but instructions can be found xref:https://www.tecmint.com/install-php-5-6-on-centos-7/[online].
Do this before the `yum` step above.
...
Be sure to pay attention to the xref:#dll_path[placement of the files after installation].
...
+
IMPORTANT: [[dll_path]]Because the extension depends on the C library (libcouchbase), the shared object `libcouchbase.so` or `libcouchbase.dll` has to be accessible by the PHP process when loading the extension.
...
For _Windows_ though, it must be copied into either into a location from the `PATH` for the PHP executable or into a directory with like executables (like apache2, IIS or php.exe).

...
If the item has already been locked, the library will raise exception with code [.var]`COUCHBASE_KEY_EXISTS` which means that the operation could not be executed temporarily, but may succeed later on.

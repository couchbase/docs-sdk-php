...
[source,php]
----
$result = $bucket->retrieveIn('user:kingarthur', 'contact.name', 'contact.email');
var_dump($result->value);
// => array(2) {
//      [0]=>
//      array(2) {
...
Within a single cluster the `counter` operation is atomic; across XDCR however, if two clients connecting to two different (bidirectional) clusters increment the counter concurrently, this may (and most likely will) result in the value only getting incremented once in total.
The same is the case for decrement.
...
[source,php]
----
$result = $bucket->counter('counter_id', 20, ['initial' => 100]);
var_dump($result->value);
// => int(100)

$result = $bucket->counter('counter_id', 1);
var_dump($result->value);
// => int(101)

$result = $bucket->counter('counter_id', -50);
var_dump($result->value);
// => int(51)
----
...
[source,php]
----
$rv = $bucket->get('counter_id');
$value = $rv->value;
$cas = $rv->cas;
if shouldIncrementValue($value) {
  $bucket->upsert('counter_id', $value + $increment_amount, ['cas' => $cas]);
}
----
...
You will find several ways of working with counters https://docs.couchbase.com/sdk-api/couchbase-php-client-2.6.1/classes/Couchbase.Bucket.html#method_counter[in the API docs].
////
...
See the xref:php-sdk::datastructures.adoc[data structures documentation] for details on implementation.
Some examples are provided below for easy reference, but more advanced Collections frameworks are accessible in xref:java-sdk::datastructures.adoc[Java] and xref:dotnet-sdk::datastructures.adoc[.NET] as well.

Data structures in Couchbase are similar in concept to data structures in, for example:

* *Map* is like PHP `Array`, and is a key-value structure, where a value is accessed by using a key string.
* *List* is also like a PHP `Array` and is a sequential data structure.
...
[source,php]
----
$bucket->mapAdd('map_id', 'name', 'John Doe');
----
...
[source,php]
----
$bucket->listGet('list_id', 0)
$bucket->mapGet('map_id', 'name') //=> 'John Doe'
----
...
[source,php]
----
$bucket->append($docid, $fragment);
$bucket->prepend($docid, $fragment);
----
...
// Store the document
$bucket->upsert('binary_doc', "\x01");

while (true) {
    // Retrieve the entire document
    $rv = $bucket->get('binary_doc');
    $value = $rv->value . "\x02";
    try {
        $bucket->replace('binary_doc', $value);
...
var_dump(bin2hex($bucket->get('binary_doc')->value));
// => string(4) "0102"
----

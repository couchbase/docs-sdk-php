...
Batching operations is done by scheduling multiple operations in sequence.
These bulk operations may be performed by passing an array as the first argument to one of the `get()`, `upsert()`, `insert()`, `replace()` and other key-value methods.
// end::intro[]

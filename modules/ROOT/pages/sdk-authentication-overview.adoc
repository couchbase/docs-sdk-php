...
Code examples for PHP can be found in xref:start-using-sdk.adoc[Getting Started with the SDK], and also as part of the xref:sdk-user-management-example.adoc[Sample Code] provided to demonstrate user management.

[#php_upgrading_to_rbac_sdk_versions]
== Upgrading to RBAC-Enabled SDK-Versions

The last pre-RBAC version of the PHP SDK was 2.3.1.
The first RBAC-enabled version was 2.3.2.
See the SDK link:/server/other-products/release-notes-archives/php-sdk[Release Notes] for more information.
...
* _Roles_ required for resource-access, and the privileges they entail, see xref:6.0@server:security:concepts-rba-for-apps.adoc[RBAC for Applications].
...
The PHP SDK has supported certificate authentication since version 2.4.3.
...
Note that you will need to load the `php_openssl` module before `php_couchbase` in your code.

...
Here is an example in PHP of manually encoding and decoding a UTF-16 string (note that to use _RAW_ format in PHP SDK
one have to substitute default transcoder with "pass through" transcoder which does nothing, read more about transcoders
in the next section):
...
$data = mb_convert_encoding('Hello, UTF-16 World!', 'UTF-8', 'UTF-16LE');
$res = $bucket->upsert('utf16_doc', $data);
var_dump($res);
// => object(Couchbase\Document)#4 (5) {
//      ["error"]=>
//      NULL
//      ["value"]=>
//      NULL
...
//      int(0)
//      ["cas"]=>
//      string(12) "bt98zvv5txj4"
//      ["token"]=>
//      NULL
//    }

$res = $bucket->get('utf16_doc');
var_dump($res);
// => object(Couchbase\Document)#5 (5) {
//      ["error"]=>
//      NULL
//      ["value"]=>
//      string(30) "效汬Ɐ唠䙔ㄭ‶潗汲Ⅴ"
//      ["flags"]=>
//      int(0)
//      ["cas"]=>
//      string(12) "bt98zvv5txj4"
...
var_dump(mb_convert_encoding($res->value, 'UTF-16LE', 'UTF-8'));
// => string(20) "Hello, UTF-16 World!"
----
...
The following shows an implementation of the encoding ([.api]`encode_value`) and decoding ([.api]`decode_value`) functions in PHP:

[source,php]
----
define('UTF16_TYPECODE', 0x0A000000);

function utf16Encoder($value) {
    $data = mb_convert_encoding($value, 'UTF-8', 'UTF-16LE');
    return [$data, UTF16_TYPECODE, 0];
}

function utf16Decoder($data, $flags, $datatype) {
    if ($flags & UTF16_TYPECODE) {
        return mb_convert_encoding($data, 'UTF-16LE', 'UTF-8');
    } else {
        return \Couchbase\defaultDecoder($data, $flags, $datatype);
    }
}

$bucket->setTranscoder('utf16Encoder', 'utf16Decoder');

$bucket->upsert('utf16_doc', 'Hello, UTF-16 World!');
$bucket->get('utf16_doc');
----
...
For example, the older legacy PHP code for _JSON_ was `0x00` and the unified typecode for _JSON_ is `0x02`.

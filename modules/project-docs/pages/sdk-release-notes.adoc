...
.Behavioral Change
====
Previously, when the application stored an instance of a string which encoded a JSON value, SDK 3.0.x would decode it as a JSON and return an object/array. 
In 3.1.0 the issue https://issues.couchbase.com/browse/PCBC-742[has been fixed], and now the application would receive the instance of the string xref:howtos:transcoders-nonjson.adoc[as expected].
====

...
Given that SDK 4.x is a combination of PHP and C++ code, we no longer use https://packagist.org/packages/couchbase/couchbase[packagist] -- which only hosts pure PHP libraries.
The code that is hosted on the packagist is for previous 3.x releases, and only implements data structures.
This code is now fully integrated into SDK 4.x.

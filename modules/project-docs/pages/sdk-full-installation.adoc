...
When using Microsoft Windows, instead of using PECL, download the pre-built binary package matching your environment.
...
From xref:sdk-release-notes.adoc#version-4-2-1-23-april-2024[PHP SDK version 4.2.1]
Windows binaries are available to download --
see the xref:sdk-release-notes.adoc[Release Notes page] for links to the binary built for your preferred PHP version.

include::sdk-release-notes.adoc[tag=pecl]

For older versions of the PHP 4.x SDK, there are no pre-built binaries for the Windows platform.
...
We recommend always running the latest version of the SDK.

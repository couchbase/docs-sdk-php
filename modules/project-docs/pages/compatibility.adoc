...
=== x86-32 Processor Support

While the SDK can be compiled for 32-bit version of x86 platform, it is not
generally tested and supported type of the deployment.

PHP does not have platform-independent
https://www.php.net/manual/en/language.types.integer.php[64-bit integer], and
all numbers are always signed. These facts do not allow to expose parts of the
Couchbase API directly, and rather perform extra conversion steps. Here are few
examples:

* JSON parser in PHP might silently convert integers in the documents to floats
  with precision loss.
* Increment/Decrement APIs in Couchbase operates on 64 bit numbers.
* CAS values are reprensented as strings right now (but it would be more
  efficient to use 64-number.
* Expiration time is essentially 32-bit number, reprensenting Unix time in
  seconds, so it has to be unsigned.

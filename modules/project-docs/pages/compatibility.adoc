...
At time of release, the Couchbase 3.2 PHP SDK is fully compatible with Couchbase Capella, our fully-hosted database-as-a-service. 
...
* Amazon Linux 2.
* Red Hat Enterprise Linux 7 (Server only -- see  <<tls-capella,below>>) & RHEL 8.
* CentOS 7 (Server only -- see <<tls-capella,below>>) & CentOS 8.
* Ubuntu (LTS) 16.04 (_Xenial_), 18.04 (_Bionic_), & 20.04 (_Focal_).
* Debian 9 (_Stretch_) & 10 (_Buster_).
* Alpine Linux (since PHP SDK 3.1.2).
...
.Capella Compatibility
====
Couchbase Capella mandates TLS 1.3 as a minimum secure standard for client connection.
TLS 1.3 needs `OpenSSL 1.1.1` -- or more recent -- on your platform.

RHEL 7x and CentOS 7 do not provide a supported way to install `OpenSSL 1.1.1` and a compatible PHP, and are thus not supported with Capella.
====

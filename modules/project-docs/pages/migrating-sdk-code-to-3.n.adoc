...
As with 2.x release, the primary source of artifacts is release notes page, where we publish links to pre-built binaries
as well as to source tarball.

SDK 3.x supports PHP interpreters starting 7.2.

Since 3.x release we start publish binaries for Windows with OpenSSL dependency. It allows the SDK simplify deployment
of the application without rebuild. Note, that OpenSSL DLLs are not distributed in the archive and must be installed
separately (see official OpenSSL page for more details).

===  Connection Lifecycle

Bootstrapping the SDK is staged now, so the application have to create Cluster object first, and then open Bucket and
Collection if necessary.

As in SDK 2.x there is not explicit shutdown, and all underlying connections still kept in the cache, for reusing in
future requests. The connection idle time is controlled by `couchbase.pool.max_idle_time_sec` PHP INI setting.

SDK 3.x allows to perform Queries on the Cluster level, so it is not necessary to open a bucket anymore.

SDK 3.x does not allow to use SASL PLAIN mechanism by default, instead it restrict to `SCRAM-SHA{1,256,512}`.

===  Exception Handling

SDK 3.x actively uses Exceptions to signal about errors. Instead of using single `\Couchbase\Exception` as in SDK 2.x,
now we use hierarchy of the exceptions, which allows to handle errors in more reliable way:
...
SDK 3.x still relies on native types and support of `json_encode` API from standard `json.so` module (therefore it still
have to be loaded before `couchbase.so`). But we stopped supporting `igbinary.so` transcoder.

===  Migrating Services

==== Key Value

Most of the KV APIs moved from bucket-level (in SDK 2.x) to collection-level (in SDK 3.x). For servers, which don't
support collections, the application could obtain default collection using `bucket->defaultCollection()` function.
...
The `BinaryCollection` mentioned above could be retrieved from regular collection object using `$collection->binary()`
method.

==== Query

In SDK 3.x API for Query was improved and now it is more consistent with other endpoints.
...
Analytics queries in SDK3 has its own API entry point.
...
Most noticeable change in Views API for SDK 3.x is the change of names of consistency control settings.

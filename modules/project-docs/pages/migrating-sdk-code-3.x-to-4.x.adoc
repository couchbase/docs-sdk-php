...
. SDKv4 uses Composer to deliver high-level API classes, while SDKv3 defines everything in the extension.
...
. SDKv4 does not expect any system libraries or headers to be installed to work,
...
SDKv4 fixes this and instead defines the `\Couchbase\Transcoder` interface, which encapsulates all of the logic.
...
SDKv4 moved exceptions into the `\Couchbase\Exception` namespace,
...
In SDKv4 various enumerations and constants changed their types from `int` to `string` to reduce potential issues.
...
SDKv3 did not allow the setting of timeouts through `\Couchbase\ClusterOptions`,

...
$country = $result->content(0); //"United Kingdom"
----
...
[source,php]
----
$result = $collection->mutateIn("some_doc", [
...
$options->cas("AABmBlI4oxU=");
$result = $collection->mutateIn("customer123", [
    new \Couchbase\MutateArrayAppendSpec("purchases.complete", [999])
], $options);
----
...
$options->cas("AABmBlI4oxU=");
$result = $collection->mutateIn("customer123", [
    new \Couchbase\MutateArrayAppendSpec("purchases.complete", [998])
], $options);
----
...
Couchbase’s xref:concept-docs:durability-replication-failure-considerations.adoc#older-server-versions[traditional 'client verified' durability], 
using `PersistTo` and `ReplicateTo`, is still available, particularly for talking to Couchbase Server 6.0 and earlier:

[source,php]
----
// 6.0 durability snippet
----

...
Common errors are listed in our xref:ref:errors-and-exceptions.adoc#analytics-errors[Errors Reference doc], with errors caused by resource unavailability (such as timeouts and _Operation cannot be performed during rebalance_ messages) leading to an xref:howtos:errors.adoc#retry[automatic retry] by the SDK.

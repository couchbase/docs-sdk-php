...
[abstract]
This section describes how to connect the PHP SDK to a Couchbase cluster.
...
This is done with the `setting-alternate-address` xref:6.6@server:cli:cbcli/couchbase-cli-setting-alternate-address.adoc[CLI command] introduced in Couchbase Server 6.5.
...
To configure encryption for the PHP SDK:
...
[source,php]
----
include::example$managing_connections.php[tag=tls,indent=0]
----
...
// DNS-SRV
include::6.6@sdk:shared:partial$dnssrv-pars.adoc[tag=dnssrv]
...
// Cloud connections
include::6.6@sdk:shared:partial$managing-connections.adoc[tag=cloud]


== Further Reading

For more on RBAC, refer to the xref:6.6@server:learn:security/authorization-overview.adoc[Server docs].

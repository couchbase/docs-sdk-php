...
+
--
Certificates from the Mozilla Root CA store are now bundled with the SDK. If the server's certificate is signed by a well-known CA (e.g., GoDaddy, Verisign, etc.), you no longer need to configure the `trust_certiciate` path in your connection string.

...
The simplest way to create a `Cluster` object is to call a new `connectionString` with a username and password:
...
Connection strings and the `__construct()` method are covered https://docs.couchbase.com/sdk-api/couchbase-php-client/classes/Couchbase-Cluster.html[in the API guide].
...
//  NB
Delete this, but add in any useful PHP-specific info

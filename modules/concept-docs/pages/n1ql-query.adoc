...
you obtain Type safety and auto completion of relevant methods and SQL++ clauses. 
...
The difference is that it emits SQL++ in place of SQL. 
...
Once you have included the dependency, you can construct queries just like any other LINQ provider in a SQL-like fashion. Nearly all SQL++ statements and functions are supported, for example:
...
In addition to SQL++ language features, Linq2Coucbase supports ORM features like Change-tracking, and mapping of JSON fields to keys and other POCO properties. 
